...
if __name__ == "__main__":
    ...
    print "Number of jobs %s" %numJobs
            ...
            if args.benchmarkindex == -1 or args.benchmarkindex == counter:
                ...
                if args.mode=="dfs":
                    if args.job == -1 or args.job == 0:
                        call_check(cmd, qsubargs + [str(0), str(0), str(limit), str(seed), "0", "dfs"] + line[4:])
                elif args.mode=="random":
                    for i in range(0, numJobs):
                        if args.job == -1 or args.job == i:
                            call_check(cmd, qsubargs + [str(0), str(0), str(limitPerJob), str(seed), str(i*limitPerJob), "random"] + line[4:])
                elif args.mode=="pct":
                    for i in range(0,numJobs):
                        if args.job == -1 or args.job == i:
                            call_check(cmd, qsubargs + [str(args.max), str(args.max), str(limitPerJob), str(seed), str(i*limitPerJob), "pct"] + line [4:])
                elif args.mode=="race":
                    os.environ["mode"]="race"
                    call_check(cmd, qsubargs + [str(0), str(args.max), str(limit), str(seed), "0", "race"] + line[4:])
                elif args.mode == "pb":
                    if args.job == -1 or args.job == 0:
                        call_check(cmd, qsubargs + [str(0), str(args.max), str(limit), str(seed), "0","pb"] + line[4:])
                elif args.mode == "db":
                    if args.job == -1 or args.job == 0:
                        call_check(cmd, qsubargs + [str(0), str(args.max), str(limit), str(seed), "0","db"] + line[4:])
                else:
                    assert(0)
            counter += 1
